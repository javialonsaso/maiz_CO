...
% https://www.backupworks.com/LTO1-Ultrium-Tape-Media.aspx


\section{Análisis de los datos}


A partir de los archivos sobre la producción de maíz (\textit{Cadena\_Productiva\_Maiz\_-\_Area\_\_Produccion\_Y\_Rendimiento.csv}
y de la temperatura media asociada a cada región del país
(\textit{tmean\_gathered.csv})
comenzamos por cargar los datos dentro de un entorno Python utilizando las librerías de \texttt{Pandas} \texttt{Numpy}. De esta forma, podemos agrupar, manipular y guardar fácilmente los datos generados.

Agrupamos por departamentos. De esta forma esperamos eliminar la variabilidad del problema proveniente de factores que no se están teniendo en cuenta, como la calidad de la tierra, fertilizantes, detalles del cuidado del cultivo, etc.

Luego crearemos un nuevo data set que relacione la temperatura y los datos del cultivo según el departamento (utilizando su identificador numérico) y el periodo de cosecha. Estos serán los datos producidos en este proyecto y tuilizaremos para obtener los resultados finales.


PONER EN TITULO
Relación entre la producción y el área cosechada
...
\begin{figure}[H]
	\includegraphics[width=0.6\linewidth]{./figures/cosecha.png} % 10x10 t=3 dt=0.001 n=10
...
\begin{figure}[H]
	\includegraphics[width=0.6\linewidth]{./figures/rendimiento.png} % 10x10 t=3 dt=0.001 n=10
...
Observamos que se produce un pico de rendimiento y ajustamos a una forma gaussiana. Aunque no es un ajuste muy adecuado (no aparecen datos alrededor de los $12\degree$C) encontramos que la temperatura óptima es de $20\degree$C. Es un valor esperado bajo, puesto que la temperatura óptima para el cultivo de maíz es de alrededor $25\degree$C. Esto puede deberse a otros factores que no se han tenido en cuenta, como la precipitación, la presión atmosférica, la temperatura de la tierra (nuestra variable independiente esla temperatura del aire en la superficie) entre otras.
